...
export function CompanyPassengers() {
  ...
  const fetchPassengers = async (opts?: {
    ...
    try {
      ...
      const passengersMapped = passengersData.map((passenger: any) => ({
        ...
        id_empresa:
          passenger.id_empresa?.toString?.() ||
          passenger.empresa_id?.toString?.() ||
          "",
        id_centro_costo:
          passenger.id_centro_costo?.toString?.() ||
          passenger.centro_costo_id?.toString?.() ||
          "",
      ...
      setPagination({
        ...
        totalPages:
          ...
          Math.ceil(
            (paginationData.total || passengersMapped.length) /
  ...
  const handleEdit = async () => {
    ...
    if (!formData.correo?.trim()) missingFields.push("Correo");
    if (!formData.telefono?.trim()) missingFields.push("Teléfono");
    if (!formData.id_empresa) missingFields.push("Empresa");
    if (!formData.id_centro_costo) missingFields.push("Centro de Costo");

    if (missingFields.length > 0) {
      setEditError(`Por favor complete: ${missingFields.join(", ")}`);
      return;
    }
    setEditError("");
    ...
    <div className="space-y-6">
                  ...
                  <PopoverTrigger asChild>
                    ...
                    >
                      {formData.id_centro_costo
                        ? costCenters.find(
                            (cc: CostCenter) =>
                              cc.id.toString() === formData.id_centro_costo,
                          )?.nombre
                          ...
                          {costCenters.map((cc) => (
                            <CommandItem
                              ...
                              onSelect={() => {
                                setFormData({
                                  ...formData,
                                  id_centro_costo: cc.id.toString(),
                                });
      ...
      <Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
        <DialogContent className="sm:max-w-[500px]">
          ...
          <div className="grid gap-4 py-4">
            ...
            <div className="space-y-2">
              ...
              <select
                ...
                onChange={(e) =>
                  setFormData({ ...formData, id_centro_costo: e.target.value })
                }
                className="w-full p-2 border rounded-md"
          ...
          {editError && (
            <p className="text-sm text-red-500 font-medium px-1">{editError}</p>
          )}
          <DialogFooter>
            <Button
              variant="outline"
